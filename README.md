...
- **Conversation viewer** — select any agent in `/agents` to open a live-scrolling overlay of its full conversation (auto-follows new content, scroll up to pause). Active tool calls show bounded argument summaries, dynamically increasing elapsed time, Bash timeout status, and a sanitized live output tail; `ctrl+o` expands or compacts the still-bounded live output. Concurrent calls stay separate, and completed results keep their normal transcript display. Steer a running agent inline by pressing `Enter` to open a composer, typing, then `Enter` to send (`Esc` or an empty submit returns) — the message appears as a user message and redirects the agent after its current tool. Stop a still-running agent by pressing `x` (then `x` again to confirm) — both work for background agents too. Assistant text renders as Markdown; `m` cycles that between off, assistant-only and everything (see [Viewer markdown](#persistent-settings))
...
| `Explore` | read, bash, grep, find, ls | configured override or inherit | `replace` (standalone) | Fast codebase exploration (read-only) |
